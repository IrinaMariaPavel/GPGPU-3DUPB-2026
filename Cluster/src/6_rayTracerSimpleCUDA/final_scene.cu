 ...
 *
 * ANSWER:
 * We launch 4 kernels: renderInit, allocateWorld, render, freeWorld.
 * - renderInit and render run with <<<blockCount, blockSize>>>, i.e. blockCount = (nx/TX, ny/TY)
 *   blocks, each with blockSize = (TX, TY) threads (one thread per pixel).
 * - allocateWorld and freeWorld run with <<<1, 1>>>, i.e. 1 block, 1 thread.
 * We need only 1 thread for allocateWorld/freeWorld because they build/destroy the world's
 * data structures (the sphere list, materials, camera) using dynamic allocation (new/delete).
 * This construction must happen exactly once; running it with multiple threads would cause
 * race conditions and duplicate/corrupted allocations.
 * The most expensive kernel is render, because it performs the actual ray tracing: for every
 * pixel it casts ns samples, and for each sample it traces a ray through up to 50 bounces,
 * testing intersections against all objects in the world. This is far more compute per thread
 * than the other kernels, which only do simple initialization or a single-threaded setup/teardown.
*/
